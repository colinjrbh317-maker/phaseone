...
import { ArrowRight, ShoppingCart, FlaskConical, ShieldCheck, Download, AlertTriangle } from "lucide-react";
import { Navbar } from "@/components/layout/navbar";
import { Footer } from "@/components/layout/footer";
import { products, testPanels } from "@/lib/constants";
      ...
      <main className="pt-24 md:pt-32 pb-20 bg-background min-h-screen text-foreground">
        <div className="max-w-7xl mx-auto px-6 lg:px-8">
          ...
          <nav className="text-xs font-bold tracking-widest uppercase text-muted-foreground mb-8">
            ...
            <Link href="/shop" className="hover:text-primary transition-colors">Pre-Orders</Link>
            <span className="mx-2">/</span>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 lg:gap-16">
            ...
            {/* Right: Product Details & Pre-order */}
            <div className="flex flex-col">
              ...
              <div className="bg-card border border-border p-5 sm:p-6 rounded-xl mb-8 space-y-4 shadow-lg">
                ...
                {/* Note: In a real app we'd make this a Client Component wrapper button to hook into context state, but statically dropping it right now for presentation */}
                <button
                  type="button"
                  className="w-full flex items-center justify-center gap-2 rounded-[4px] gradient-button h-14 text-sm sm:text-base font-black uppercase tracking-widest glow-primary-hover shadow-[0_0_20px_rgba(0,191,255,0.3)] transition-all"
                >
              ...
              <div className="space-y-4 border-t border-border pt-8">
                <h3 className="font-heading font-black uppercase tracking-tighter text-xl mb-4">Independent Testing Specifications</h3>
                {testPanels.map((panel) => (
                  <div key={panel.key} className="p-4 border border-border/50 rounded-lg bg-card/30 hover:border-primary/30 transition-colors">
                    <div className="flex items-center gap-2 mb-2">
                      <ShieldCheck size={16} className="text-primary drop-shadow-[0_0_8px_rgba(0,191,255,0.8)]" />
                      <h4 className="font-bold text-sm uppercase tracking-widest text-foreground">{panel.fullName}</h4>
                    </div>
