...
import { researchGroups, type Product } from "@/lib/constants";
import { ProductDrawerModal } from "@/components/ui/product-drawer-modal";

export type ShopContentProduct = Product & {
...
function ProductGridCard({
  ...
  return (
    <button
      onClick={onClick}
      disabled={outOfStock}
      className="group relative flex flex-col rounded-2xl border border-border bg-background hover:border-primary/40 hover:shadow-[0_16px_40px_hsl(32,100%,70%,0.18)] transition-all duration-500 overflow-hidden text-left w-full h-full disabled:opacity-60"
    >
      <div className="relative w-full aspect-square bg-gradient-to-br from-primary/5 via-white to-secondary/5 overflow-hidden">
        ...
        {outOfStock && (
          <span className="absolute top-3 right-3 z-10 inline-flex items-center rounded-full bg-background border border-muted-foreground/40 px-2.5 py-0.5 text-[10px] font-semibold text-muted-foreground uppercase tracking-wide">
            Out of Stock
          </span>
        )}
      ...
      <div className="p-4 flex flex-col gap-2 min-h-[140px] flex-1">
        <h3 className="font-heading font-black tracking-tighter uppercase text-base text-foreground group-hover:text-primary transition-colors">
          {product.name}
        </h3>
...
function ShopGrid({ products }: { products: ShopContentProduct[] }) {
  const searchParams = useSearchParams();
  const q = (searchParams.get("q") ?? "").toLowerCase();
  const groupFilter = searchParams.get("group") ?? "";

  const [selectedProduct, setSelectedProduct] =
    useState<ShopContentProduct | null>(null);

  const filtered = useMemo(() => {
    let list = products;
    if (q) {
      list = list.filter((p) =>
        `${p.name} ${p.slug} ${p.description}`.toLowerCase().includes(q),
      );
    }
    if (groupFilter) {
      ...
      <main className="pt-24 md:pt-32 pb-20 bg-background min-h-screen">
        <div className="max-w-7xl mx-auto px-6 lg:px-8">
          ...
          <div className="mb-12 max-w-3xl">
            <h1 className="font-heading font-black tracking-tighter uppercase text-4xl sm:text-5xl md:text-6xl text-foreground mb-5">
              Research{" "}
              <em className="not-italic">
                <span className="italic text-primary drop-shadow-[0_0_20px_rgba(0,191,255,0.4)]">
                  Catalog
                </span>
              </em>
            </h1>
...
export function ShopContent({ products }: { products: ShopContentProduct[] }) {
  return (
